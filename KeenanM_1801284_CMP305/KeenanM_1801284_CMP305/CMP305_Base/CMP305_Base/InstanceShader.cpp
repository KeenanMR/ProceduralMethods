...
InstanceShader::InstanceShader( ID3D11Device* device, HWND hwnd ) : BaseShader( device, hwnd ) {
	initShader( L"instanced_vs.cso", L"instanced_ps.cso" );
}
...
void InstanceShader::setShaderParameters( ID3D11DeviceContext* deviceContext, const XMMATRIX &worldMatrix, const XMMATRIX &viewMatrix, const XMMATRIX &projectionMatrix, ID3D11ShaderResourceView* texture, ID3D11ShaderResourceView* texture2, ID3D11ShaderResourceView* texture3, ID3D11ShaderResourceView* texture4, ID3D11ShaderResourceView* texture5, Light* light ) {
	HRESULT result;
